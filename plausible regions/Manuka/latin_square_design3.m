...
tic 
p=10;%number of parameters to estimate %tau=0 death rates for 2 groups ar ethe same 
...
lx=[lx(:,1:6) zeros(n*p,1) lx(:,7:8) lx(:,7) lx(:,9:10)]; %to make sure tau=0, par 8,10=are the same death rates are zero
    
x=zeros(n*p,p+1); %parameter space 
%transmission rates and removal rates p 1,9, 11:18
%ui=[repmat([0.001 5],4,1);repmat([0.001 5],6,1)];
%ui=repmat([0.0001 10],4,1);
ui=[0.0001 2;0.0001 4; 0.0001 1; 0.0001 1.5];

x(:,[1 9 11:12])=(lx(:,[1 9 11:12]).* repmat((ui(:,2)-ui(:,1))',n*p,1))+repmat(ui(:,1)',n*p,1);
...
J=5; %number of summary stats to check

%simulator:
k=50; %number of times the simulator runs 
