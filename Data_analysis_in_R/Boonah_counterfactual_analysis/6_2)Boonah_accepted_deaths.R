...
#hierarchical model:
hi1=readMat("INT_d_Q.mat")
hi1=hi1$INT.d.Q

hi2=readMat("INT_d_B.mat")
hi2=hi2$INT.d.B
...
#hie_paths_m=data.frame(melt(hie_paths2),rep("Interventions",2000))
#no interventions 

sc_paths=readMat('NO_d_B.mat')
sc_paths=sc_paths$NO.d.B
sc_paths=colSums(sc_paths)

#sc_m=melt(sc_paths)
#sc_paths_m=data.frame(sc_m,rep("No interventions",2000))



path_diff=sc_paths-hie_paths2
...
path_diff_his_acc=ggplot(path_diff_m,aes(x=value))+
  geom_histogram(binwidth=4,fill="#4E2A1E",alpha=0.8)+
...
All_pred<-ggarrange(path_diff_his_acc,path_diff_his_pred,ncol = 2,labels = c("Accepted","Predicted"),common.legend = T)
...
ggsave('boonah_acc_pred_deaths.png',last_plot(),height = 6,width = 8)
